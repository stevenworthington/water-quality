...
####################################################################################
####################################################################################
# 1) Model for functionality (ref 8)
...
# remove "openallday"
X_mat8_list_func <- X_mat8_list
for (i in 1:length(X_mat8_list_func)) {
	X_mat8_list_func[[i]] <- X_mat8_list_func[[i]][, !(colnames(X_mat8_list_func[[i]]) %in% "x3_1_b_openallday1")]
}

# alpha=1 for lasso only
for (i in seq_along(X_mat8_list_func)) {
	print(i)
    # use CV to select best lambda
    cv_glmmod_func[[i]] <- cv.glmnet(X_mat8_list_func[[i]], y=factor(imp8_list[[i]][-c(100, 221), "functionalinferred"]),
...
for (i in seq_along(X_mat8_list_func)) {
    glm_mat8_func[[i]] <- data.frame(X_mat8_list_func[[i]], functionalinferred=imp8_list[[i]][-c(100, 221), "functionalinferred"])
}
...
functional_models8 <- lapply(glm_mat8_func, function(x) {
	geeglm(func_form, family = poisson(link = "log"), data = x, id = 1:352, corstr = "exchangeable") 
	})

# pool results
functional_models8_pooled <- pool(functional_models8)

# summarize results
functional_results8 <- summary(functional_models8_pooled) 

# calculate RR point and interval estimates
functional_results8 <- functional_results8 %>%
    mutate(RR = exp(estimate),
           RR_lcl = exp(estimate - (1.96*std.error)),
           RR_ucl = exp(estimate + (1.96*std.error))
           )

write.csv(functional_results8, file = "results/functional_results_ref8.csv", row.names = FALSE)
...
for (i in seq_along(imp8_list)) {
    imp8_list[[i]][, "ttc_11"] <- factor(ifelse(imp8_list[[i]][, "ttc"] >= 11, "high", "low"), levels = c("low", "high"))
    imp8_list[[i]][, "ttc_101"] <- factor(ifelse(imp8_list[[i]][, "ttc"] >= 101, "high", "low"), levels = c("low", "high"))
}
...
# alpha=1 for lasso only
for (i in seq_along(X_mat8_list)) {
	print(i)
    # use CV to select best lambda
    cv_glmmod_ttc11[[i]] <- cv.glmnet(X_mat8_list[[i]], y=factor(imp8_list[[i]][-c(100, 221), "ttc_11"]),
                                      alpha=1, family="binomial", nfolds=10, parallel=FALSE)
...
for (i in seq_along(X_mat8_list)) {
    glm_mat8_ttc11[[i]] <- data.frame(X_mat8_list[[i]], ttc_11=as.numeric(imp8_list[[i]][-c(100, 221), "ttc_11"])-1)
}
...
# fit complete-data model
ttc11_models8 <- lapply(glm_mat8_ttc11, function(x) {
	geeglm(ttc11_form, family = poisson(link = "log"), data = x, id = 1:352, corstr = "exchangeable")  
	})

# pool results
ttc11_models_pooled8 <- pool(ttc11_models8)

# summarize results
ttc11_results8 <- summary(ttc11_models_pooled8) 

# calculate RR point and interval estimates
ttc11_results8 <- ttc11_results8 %>%
    mutate(RR = exp(estimate),
                  RR_lcl = exp(estimate - (1.96*std.error)),
                  RR_ucl = exp(estimate + (1.96*std.error))
                  )

write.csv(ttc11_results8, file = "results/ttc11_results_ref8.csv", row.names = FALSE)
...
for (i in seq_along(X_mat8_list)) {
	print(i)
    # use CV to select best lambda
    cv_glmmod_ttc101[[i]] <- cv.glmnet(X_mat8_list[[i]], y=factor(imp8_list[[i]][-c(100, 221), "ttc_101"]),
                                       alpha=1, family="binomial", nfolds=10, parallel=FALSE)
...
# combine new design matrix
glm_mat8_ttc101 <- list()

for (i in seq_along(X_mat8_list)) {
    glm_mat8_ttc101[[i]] <- data.frame(X_mat8_list[[i]], ttc_101=as.numeric(imp8_list[[i]][-c(100, 221), "ttc_101"])-1)
}
...
# fit complete-data model
ttc101_models8 <- lapply(glm_mat8_ttc101, function(x) {
	geeglm(ttc101_form, family = poisson(link = "log"), data = x, id = 1:352, corstr = "exchangeable")   
	})

# pool results
ttc101_models_pooled8 <- pool(ttc101_models8)

# summarize results
ttc101_results8 <- summary(ttc101_models_pooled8) 

# calculate RR point and interval estimates
ttc101_results8 <- ttc101_results8 %>%
    mutate(RR = exp(estimate),
                  RR_lcl = exp(estimate - (1.96*std.error)),
                  RR_ucl = exp(estimate + (1.96*std.error))
                  )

write.csv(ttc101_results8, file = "results/ttc101_results_ref8.csv", row.names = FALSE)
...
for (i in seq_along(X_mat8_list)) {
	print(i)
    # use CV to select best lambda
    cv_glmmod_enoughwater[[i]] <- cv.glmnet(X_mat8_list[[i]], y=factor(imp8_list[[i]][-c(100, 221), "x3_6_enoughwater"]),
                                            alpha=1, family="binomial", nfolds=10, parallel=FALSE)
...
# combine new design matrix
glm_mat8_enoughwater <- list()

for (i in seq_along(X_mat8_list)) {
    glm_mat8_enoughwater[[i]] <- data.frame(X_mat8_list[[i]], x3_6_enoughwater=as.numeric(imp8_list[[i]][-c(100, 221), "x3_6_enoughwater"])-1)
}
...
# fit complete-data model
enoughwater_models8 <- lapply(glm_mat8_enoughwater, function(x) {
	geeglm(enoughwater_form, family = poisson(link = "log"), data = x, id = 1:352, corstr = "exchangeable")  
	})

# pool results
enoughwater_models_pooled8 <- pool(enoughwater_models8)

# summarize results
enoughwater_results8 <- summary(enoughwater_models_pooled8) 

# calculate RR point and interval estimates
enoughwater_results8 <- enoughwater_results8 %>%
    mutate(RR = exp(estimate),
                  RR_lcl = exp(estimate - (1.96*std.error)),
                  RR_ucl = exp(estimate + (1.96*std.error))
                  )

write.csv(enoughwater_results8, file = "results/enoughwater_results_ref8.csv", row.names = FALSE)


####################################################################################
####################################################################################
# 3) Model for length of last breakdown - ref 8
...
X_mat8_list_sub <- list()

for (i in seq_along(X_mat8_list)) {
	X_mat8_list_sub[[i]] <- X_mat8_list[[i]][keep_obs, !(colnames(X_mat8_list[[i]]) %in% c("x5_1_a_facilitator1", "x5_0_latrineaccess", "x3_1_b_openallday1"))]
}
...
for (i in seq_along(X_mat8_list)) {
	print(i)
    # use CV to select best lambda
    cv_glmmod_breakdown[[i]] <- cv.glmnet(X_mat8_list_sub[[i]], y=x3_3_a_lengthbreakdown_ln,
...
# combine new design matrix
glm_mat8_breakdown <- list()

for (i in seq_along(X_mat8_list)) {
    glm_mat8_breakdown[[i]] <- data.frame(X_mat8_list_sub[[i]], x3_3_a_lengthbreakdown_ln=x3_3_a_lengthbreakdown_ln)
}
...
# fit complete-data model
breakdown_models8 <- lapply(glm_mat8_breakdown, function(x) {
	lm(breakdown_form, data = x) 
	})

# pool results
breakdown_models_pooled8 <- pool(breakdown_models8)

# summarize results
breakdown_results8 <- summary(breakdown_models_pooled8) 

# calculate interval estimates
breakdown_results8 <- breakdown_results8 %>%
    mutate(lcl = estimate - (1.96*std.error),
           ucl = estimate + (1.96*std.error)
           )

write.csv(breakdown_results8, file = "results/breakdown_results_ref8.csv", row.names = FALSE)
